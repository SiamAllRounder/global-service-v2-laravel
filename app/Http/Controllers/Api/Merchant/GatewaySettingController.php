...
class GatewaySettingController extends Controller
{
    public function index()
    {
        ...
        $message = ['success' => ['Gateway Settings']];
    ...
    public function updateWalletStatus(Request $request)
    {
        ...
        if($request->status == true &&  $setting->wallet_status == true){
            $error = ['error'=>['Your Wallet Balance System Already Enabled']];
            return Helpers::error($error);
        }
        if($request->status == false &&  $setting->wallet_status == false){
            $error = ['error'=>['Your Wallet Balance System Already Disabled']];
            return Helpers::error($error);
        }
        try{
        ...
        if($request->status == true ){
            $message = ['success' => ['Wallet Balance System Enabled Successfully']];
            return Helpers::onlysuccess($message);
        }else{
            $message = ['success' => ['Wallet Balance System Disabled Successfully']];
            return Helpers::onlysuccess($message);
        }
    }
    public function updateVirtualCardStatus(Request $request)
    {
        ...
        if($request->status == true &&  $setting->virtual_card_status == true){
            $error = ['error'=>['Your Virtual Card System Already Enabled']];
            return Helpers::error($error);
        }
        if($request->status == false &&  $setting->virtual_card_status == false){
            $error = ['error'=>['Your Virtual Card System Already Disabled']];
            return Helpers::error($error);
        }
        try{
            $status = $request->status;
        ...
        if($request->status == true ){
            $message = ['success' => ['Virtual Card System Enabled Successfully']];
            return Helpers::onlysuccess($message);
        }else{
            $message = ['success' => ['Virtual Card System Disabled Successfully']];
            return Helpers::onlysuccess($message);
        }
    }
    public function updateMasterCardStatus(Request $request)
    {
        ...
        if($request->status == true &&  $setting->master_visa_status == true){
            $error = ['error'=>['Your Master/Visa System Already Enabled']];
            return Helpers::error($error);
        }
        if($request->status == false &&  $setting->master_visa_status == false){
            $error = ['error'=>['Your Master/Visa System Already Disabled']];
            return Helpers::error($error);
        }
        try{
            $status = $request->status;
        ...
        if($request->status == true ){
            $message = ['success' => ['Master/Visa System Enabled Successfully']];
            return Helpers::onlysuccess($message);
        }else{
            $message = ['success' => ['Master/Visa System Disabled Successfully']];
            return Helpers::onlysuccess($message);
        }
    }
    public function updateMasterCardCredentials(Request $request)
    {
        ...
        $setting->save();
        $message = ['success' => ['Master/Visa card credentials updated successfully!']];
        return Helpers::onlysuccess($message);
